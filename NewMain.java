...
// Get resource is required for finding the image within the JAR achive once packed
    final public ImageIcon icon = (new ImageIcon(getClass().getResource("Java.jpg")));
...
//Creation of JPanels to be added to the frame
    ...
    public NewMain() // Constructor Method for GUI
    {
        ...
        setTitle("Pallavi & Rupa's Cinema Booking System"); // Set Title of Main Window

        setSize(800, 500); // Set frame resolution to [x,y] pixels
...
// Addition of Content to respective Panel (Order determines position within panel)
        ...
        bannerPanel.add(iconHolder, BorderLayout.NORTH);
        bannerPanel.add(p0, BorderLayout.CENTER);
        
        p0.add(ticketQuantityTitle);
        p0.add(ticketQuantityValue);
        p0.add(ticketTypeCostTitle);
        p0.add(ticketTypeCostValue);
...
// Create New Instance of the Database class
           FullDatabaseGeneration db = new FullDatabaseGeneration();
...
// then state that not all tickets have been assined seats //12
...
//try/catch end, if error- prints message to command line followed by error code
                    ...
                    System.err.println("Error in database manipulation, code: " + ex.getMessage());
                }
...
// Pass this value to 'Ticket.getSeatPrice' method to obtain price
        int childTicketPrice = Tickets.getSeatPrice(childInput, "child");

        int adultTicketPrice = Tickets.getSeatPrice(adultInput, "adult");

        int oapTicketPrice = Tickets.getSeatPrice(oapInput, "oap");

        System.out.println("ChildPrice: "   + childTicketPrice   +
                           "adultPrice: "   + adultTicketPrice   +
                           "oapPrice: "     + oapTicketPrice);
        
// Calculate total
        int total = (childTicketPrice + adultTicketPrice + oapTicketPrice);

//Convert this integer value to a string in the correct format
        String totalString = Tickets.calculateStringTotal(total);

        System.out.println("Total Price String: " + totalString);
...
//DRAW RIGHT BLOCK OF SEATS
    ...
    public static void main(String[] args) // Main Method Declaration
    {
        ...
        new NewMain(); // Create a new Instance of the main Program when ran
